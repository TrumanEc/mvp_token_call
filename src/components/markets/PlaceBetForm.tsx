...
export function PlaceBetForm({ marketId, userId, userBalance, odds, onSuccess }: PlaceBetFormProps) {
  ...
  useEffect(() => {
    const fetchQuote = async () => {
      ...
      try {
        const res = await fetch(`/api/markets/${marketId}/price-quote?side=${side}&amount=${amountNum}`)
  ...
  const avgPrice = quote ? quote.avgPrice : (side === 'YES' ? odds.yesOdds/100 : odds.noOdds/100)
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          <div className="text-2xl font-bold">
            {quote ? (quote.newProbabilities.yes * 100).toFixed(1) : odds.yesOdds.toFixed(1)}%
          </div>
          <div className="text-sm font-medium">YES</div>
          <div className="text-xs opacity-75">
             Price: ${(quote ? quote.newProbabilities.yes : odds.yesOdds/100).toFixed(2)}
          </div>
        </button>
        ...
        >
          <div className="text-2xl font-bold">
             {quote ? (quote.newProbabilities.no * 100).toFixed(1) : odds.noOdds.toFixed(1)}%
          </div>
          <div className="text-sm font-medium">NO</div>
          <div className="text-xs opacity-75">
             Price: ${(quote ? quote.newProbabilities.no : odds.noOdds/100).toFixed(2)}
          </div>
        </button>
      </div>
      ...
      <div className="space-y-2 text-sm text-gray-600 bg-gray-50 p-3 rounded-md">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-medium">
             {quoteLoading ? 'Calculando...' : `$${avgPrice.toFixed(3)}`}
          </span>
        </div>
        <div className="flex justify-between text-base border-t pt-2 mt-2">
          <span className="font-semibold text-gray-800">Acciones Estimadas:</span>
          <span className="font-bold text-gray-900">
             {quoteLoading ? '...' : (quote?.shares.toFixed(2) || '0.00')}
          </span>
        </div>
        <div className="flex justify-between text-base">
          <span className="font-semibold text-green-700">Retorno Potencial:</span>
      ...
      {quote?.wouldExceedCap && (
        <div className="bg-orange-50 border border-orange-200 p-3 rounded-md text-orange-800 text-sm">
          ...
          <p>{quote.capReason}</p>
          <p className="mt-1">
            Puedes comprar hasta <strong>${quote.maxAllowedAmount.toFixed(2)}</strong> por el momento.
          </p>
          <button
            type="button"
      ...
      <Button
        ...
        disabled={loading || quoteLoading || amountNum <= 0 || !!quote?.wouldExceedCap}
      >
        Comprar {side}
      </Button>
    </form>
