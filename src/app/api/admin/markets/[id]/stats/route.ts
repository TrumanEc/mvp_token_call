...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      simulation: {
        ifYesWins: { payoutPerDollar: yes.isZero() ? 0 : netPool.dividedBy(yes).toNumber() },
        ifNoWins: { payoutPerDollar: no.isZero() ? 0 : netPool.dividedBy(no).toNumber() },
        platformCommission: total.times(fee).toNumber()
      },
      priceHistory: (market as any).history?.map((h: any) => ({
        id: h.id,
      ...
      purchases: (market as any).positions?.map((p: any) => ({
        id: p.id,
